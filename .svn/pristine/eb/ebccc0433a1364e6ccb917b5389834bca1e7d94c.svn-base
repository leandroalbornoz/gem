...
<?php $salto_pagina = FALSE; ?>
<?php if (!empty($casos['liquidacion_sin_servicio'])): ?>
	<?php if ($salto_pagina): ?>
		<div style="page-break-before: always;"></div>
	<?php else: ?>
		<?php $salto_pagina = TRUE; ?>
	<?php endif; ?>
					...
					<tbody>
						<?php foreach ($casos['liquidacion_sin_servicio'] as $fila): ?>
							<tr style="font-size:16px;">
								<td>
									<span style="font-size:18px;"><?= $fila->cuil ? "$fila->cuil (" . substr($fila->liquidacion_s, -2) . ")" : $fila->liquidacion_s; ?></span>
									<br/>
								...
								<td style="font-size:16px;">
									<?= empty($fila->fechaini) ? '' : date_format(new DateTime(substr($fila->fechaini, -4) . substr($fila->fechaini, 0, 2) . '01'), 'd/m/y'); ?>
								</td>
								<td>
								</td>
							</tr>
