<script>
	...
	function actualizarRoles() {
		$("#roles").html('');
		$.ajax({
			url: 'division_inasistencia/modal_buscar_usuarios/<?= $division->id; ?>',
			data: {
				usuario: userSelect
			},
			type: 'POST',
			...
			success: function(data) {
				...
				if (data.roles === '-1') {
					...
					if (data.rol_inasistencia === '0' && data.rol_docente_division === '0') {
						html_data += '</tbody><tfoot><tr><th style="text-align: center" colspan="2"><a class="btn btn-primary" href="division_inasistencia/asignar_rol_asistencia_division/' + userSelect + '/' + division_id + '" title="Asignar rol asistencia"><i class="fa fa-pencil"></i> Asignar rol asistencia</a></th></tr></tfoot>';
						html_data += '</tbody><tfoot><tr><th style="text-align: center" colspan="2"><a class="btn btn-primary" href="division/asignar_rol_docente_division/' + userSelect + '/' + division_id + '" title="Asignar rol docente"><i class="fa fa-pencil"></i> Asignar rol docente</a></th></tr></tfoot>';
					} else if (data.rol_inasistencia === '1' && data.rol_docente_division === '0') {
						html_data += '</tbody><tfoot><tr><th style="text-align: center" colspan="2"><a class="btn btn-success disabled"><i class="fa fa-ban"></i> Rol asistencia ya asignado</a></th></tr></tfoot>';
						html_data += '</tbody><tfoot><tr><th style="text-align: center" colspan="2"><a class="btn btn-primary" href="division/asignar_rol_docente_division/' + userSelect + '/' + division_id + '" title="Asignar rol docente"><i class="fa fa-pencil"></i> Asignar rol docente</a></th></tr></tfoot>';
					} else if (data.rol_inasistencia === '0' && data.rol_docente_division === '1') {
						html_data += '</tbody><tfoot><tr><th style="text-align: center" colspan="2"><a class="btn btn-primary" href="division_inasistencia/asignar_rol_asistencia_division/' + userSelect + '/' + division_id + '" title="Asignar rol asistencia"><i class="fa fa-pencil"></i> Asignar rol asistencia</a></th></tr></tfoot>';
						html_data += '</tbody><tfoot><tr><th style="text-align: center" colspan="2"><a class="btn btn-success disabled"><i class="fa fa-ban"></i> Rol docente ya asignado</a></th></tr></tfoot>';
					} else {
						html_data += '</tbody><tfoot><tr><th style="text-align: center" colspan="2"><a class="btn btn-success disabled"><i class="fa fa-ban"></i> Rol asistencia ya asignado</a></th></tr></tfoot>';
						html_data += '</tbody><tfoot><tr><th style="text-align: center" colspan="2"><a class="btn btn-success disabled"><i class="fa fa-ban"></i> Rol docente ya asignado</a></th></tr></tfoot>';
					}
...
<div class="modal-body">
	<div class="row">
		<div class="col-sm-12">
			<p>Se asignará el rol de asistencia de alumno al usuario:</p>
		</div>
	</div>
	<form>
		<div class="row">
