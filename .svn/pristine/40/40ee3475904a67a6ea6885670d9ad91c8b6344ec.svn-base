...
function _mergeFullCSS($p, &$t, $tag, $classes, $id) {
		...
		// STYLESHEET CLASS e.g. #smallone{}  #redletter{}
		if (isset($id) && $id) {
		  if(isset($p['ID>>'.$id])) $this->_mergeCSS($p['ID>>'.$id], $t);
		}
		// STYLESHEET CLASS e.g. .smallone{}  .redletter{}
		foreach($classes AS $class) {
		...
		// STYLESHEET CLASS e.g. #smallone{}  #redletter{}
		if (isset($id)) {
		  if(isset($p[$tag.'>>ID>>'.$id]))$this->_mergeCSS($p[$tag.'>>ID>>'.$id], $t);
		}
}
...
/*-- END LISTS --*/
	...
	if ($inherit == 'BLOCK') {
		...
		//===============================================
		if(isset($this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS']) && isset($this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS']))
