...
class Bajas extends MY_Controller {
	...
	public function aud_escuela($escuela_id = NULL, $planilla_tipo_id = NULL, $mes = NULL) {
		...
		$data['planilla_tipo'] = $planilla_tipo;
		$data['mes'] = $mes;
		$bajas = $this->bajas_model->get_bajas_escuela($planilla_tipo->id, $mes, $escuela->id);
		foreach ($bajas as $baja) {
			$data['bajas_estado'][$baja->estado][] = $baja;
		}
		...
		if (AMES_LIQUIDACION === $mes) {
			...
			$data['casos_revisar'] = $this->tbcabh_model->get_casos_aud_bajas($escuela, AMES_LIQUIDACION, $planilla_tipo_id);
		}
	...
	public function modal_pasar_auditoria($escuela_id = NULL, $mes = NULL, $servicio_id = NULL) {
		...
		$this->servicio_novedad_model->fields['dias']['label'] = 'Días a descontar';
		$this->servicio_novedad_model->fields['obligaciones']['label'] = 'Obl. a descontar';
		$this->load->model('servicio_model');
		...
		$this->load->model('tbcabh_model');
		$caso = $this->tbcabh_model->es_caso_aud_bajas($escuela, AMES_LIQUIDACION, $servicio);
		if (empty($caso)) {
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			if (empty($servicio->fecha_baja)) {
				...
				$observaciones = (empty($servicio->observaciones) ? '' : "$servicio->observaciones ") . 'F. Baja: ' . (new DateTime($servicio->fecha_baja))->format('d/m/Y');
			}
			$trans_ok &= $this->servicio_model->update(array(
				'id' => $servicio->id,
				'fecha_baja' => (new DateTime($mes . '01 +1 month -1 day'))->format('Y-m-d'),
				'motivo_baja' => '02-4 BAJA SERVICIO - FIN DE REEMPLAZO',
		...
		$data['title'] = "Pasar a auditoría baja de <b>$servicio->apellido, $servicio->nombre</b> ($servicio->cuil)";
		$this->load->view('liquidaciones/bajas/bajas_modal_pasar_auditoria', $data);
	}
