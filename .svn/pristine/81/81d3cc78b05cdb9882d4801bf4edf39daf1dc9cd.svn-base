...
class Usuario extends MY_Controller {
	...
	function __construct() {
		...
		$this->roles_permitidos = array(ROL_ADMIN, ROL_USI, ROL_PRIVADA, ROL_SEOS, ROL_LINEA, ROL_GRUPO_ESCUELA, ROL_JEFE_LIQUIDACION, ROL_LIQUIDACION, ROL_DIR_ESCUELA);
	...
	public function listar_data($rol_codigo = NULL, $entidad_id = '') {
		...
		switch ($this->rol->codigo) {
			...
			case ROL_JEFE_LIQUIDACION:
				$this->datatables
					->where("rol.codigo IN ('" . ROL_CONSULTA_LINEA . "', '" . ROL_REGIONAL . "', '" . ROL_LINEA . "','" . ROL_GRUPO_ESCUELA . "','" . ROL_GRUPO_ESCUELA_CONSULTA . "', '" . ROL_DIR_ESCUELA . "', '" . ROL_SUPERVISION . "', '" . ROL_AREA . "', '" . ROL_LIQUIDACION . "')")
			...
			case ROL_LIQUIDACION:
				$this->datatables
					->where("rol.codigo IN ('" . ROL_CONSULTA_LINEA . "', '" . ROL_REGIONAL . "', '" . ROL_LINEA . "', '" . ROL_GRUPO_ESCUELA . "', '" . ROL_GRUPO_ESCUELA_CONSULTA . "', '" . ROL_DIR_ESCUELA . "', '" . ROL_SUPERVISION . "', '" . ROL_AREA . "')")
					->add_column('edit', '<a class="btn btn-xs btn-primary" href="usuario/editar_roles/$1" title="Editar roles"><i class="fa fa-pencil"></i> Editar roles</a>', 'id');
				break;
			case ROL_DIR_ESCUELA:
				$this->datatables
					->where("rol.codigo IN ('" . ROL_ESCUELA_ALUM . "', '" . ROL_ESCUELA_CAR . "', '" . ROL_DIR_ESCUELA . "', '" . ROL_BONO_SECUNDARIO . "') AND usuario_rol.entidad_id = {$this->rol->entidad_id}")
					->add_column('edit', '<a class="btn btn-xs btn-primary" href="usuario/editar_roles/$1" title="Editar roles"><i class="fa fa-pencil"></i> Editar roles</a>', 'id');
				break;
			default:
		}
...
//		}
	...
	public function editar_persona($id, $cuil, $usuario_id) {
		if (!accion_permitida($this->rol, $this->roles_permitidos) || $id == NULL || !ctype_digit($id)) {
			show_error('No tiene permisos para la acción solicitada', 500, 'Acción no autorizada');
		}
	...
	public function editar($id = NULL) {
		if (!accion_permitida($this->rol, $this->roles_admin) || $id == NULL || !ctype_digit($id)) {
			show_error('No tiene permisos para la acción solicitada', 500, 'Acción no autorizada');
		}
