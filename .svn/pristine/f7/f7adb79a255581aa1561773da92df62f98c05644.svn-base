...
//		}
		...
		if ($alumno_egreso->cantidad > 0) {
			$alertas[] = new Alerta('Confirmar salida de alumnos', $alumno_egreso->cantidad, "alertas/alumno_egreso/$escuela_id", 'alumnos');
		}
...
//		}
		$modulo_extintores = $this->db->select('ee.extintor_relevamiento_id')
				->from('extintor_escuela ee')
				->join('extintor e', 'ee.extintor_relevamiento_id=e.extintor_relevamiento_id and ee.escuela_id=e.escuela_id', 'left')
				->where('ee.escuela_id', $escuela_id)
				->where('ee.extintores_faltantes', '0')
				->where("COALESCE(ee.observaciones, '')=''")
				->group_by('ee.extintor_relevamiento_id')
				->having('COUNT(DISTINCT e.id)=0')
				->order_by('ee.extintor_relevamiento_id DESC')
				->get()->row();
		if (!empty($modulo_extintores)) {
			$alertas[] = new Alerta('Relevamiento Extintores sin cargar', '1', "extintores/extintor/ver/$modulo_extintores->extintor_relevamiento_id/$escuela_id", 'cargos');
		}
		$modulo_conectividad = $this->db->select('cne.conectividad_nacion_id, cne.celular_contacto')
				->from('conectividad_nacion_escuela cne')
				->join('conectividad_nacion_encargado cnp', 'cne.id=cnp.conectividad_nacion_escuela_id', 'left')
				->where('cne.escuela_id', $escuela_id)
				->group_by('cne.conectividad_nacion_id')
				->having('COUNT(DISTINCT cnp.id)=0')
				->or_having("COALESCE(cne.celular_contacto, '')=''")
				->order_by('cne.conectividad_nacion_id DESC')
				->get()->row();
		if (!empty($modulo_conectividad)) {
			$alertas[] = new Alerta('Instalación Conectividad sin cargar', '1', "conectividad/instalacion/ver/$modulo_conectividad->conectividad_nacion_id/$escuela_id", 'cargos');
		}
		return $alertas;
	}

	public function get_alertas_escuela_planilla($escuela_id) {
		$alertas = array();
		if (empty($escuela_id)) {
			return array();
		}
		$modulo_conectividad = $this->db->select('cne.conectividad_nacion_id, cne.celular_contacto')
				...
				->get()->row();
		if (!empty($modulo_conectividad)) {
			$alertas[] = new Alerta('Encargado/Celular contacto para Plan Nacional de Conectividad sin cargar', '1', "conectividad/instalacion/ver/$modulo_conectividad->conectividad_nacion_id/$escuela_id", 'cargos');
		}
		return $alertas;
	}
}
