...
class Eleccion_desinfeccion_model extends MY_Model {
	...
	public function __construct() {
		...
		$this->default_join = array(
			...
			array("(SELECT eleccion_desinfeccion_id, COUNT(1) cantidad FROM eleccion_desinfeccion_persona WHERE estado = 'Activo' GROUP BY eleccion_desinfeccion_id) p", 'p.eleccion_desinfeccion_id = eleccion_desinfeccion.id', 'left', array("p.cantidad as celadores_asignados"))
		);
	}
	...
	public function get_vista_supervision($supervision_id, $data) {
		...
		$elecciones = $this->db->select('eleccion.id, eleccion.descripcion as descripcion')
				->from('eleccion')
				->get()->result();
		$escuelas_desinfeccion = $this->db->select("e.id, CONCAT((e.numero),' - ',(e.nombre)) as escuela, e.anexo, ed.celadores_permitidos, ed.fecha_cierre as fecha_cierre , ed.mesas, eleccion.descripcion as eleccion, eleccion.id as eleccion_id, COALESCE(p.cantidad,0) as celadores_asignados")
				->from('eleccion_desinfeccion ed')
				->join('escuela e', 'ed.escuela_id = e.id', 'left')
				->join('supervision s', 'e.supervision_id = s.id AND s.id = ' . $supervision_id, 'left')
				->join('eleccion', 'ed.eleccion_id = eleccion.id', 'left')
				...
				->order_by('eleccion.id, e.numero, e.anexo')
				->get()->result();
		$escuelas_desinfeccion_eleccion = array();
		foreach ($elecciones as $eleccion) {
			foreach ($escuelas_desinfeccion as $escuela) {
				if ($eleccion->descripcion === $escuela->eleccion) {
					$escuelas_desinfeccion_eleccion[$eleccion->descripcion][] = $escuela;
				}
			}
		}
		$return['escuelas_desinfeccion_eleccion'] = $escuelas_desinfeccion_eleccion;
		$return['escuelas_desinfeccion'] = $escuelas_desinfeccion;
	...
	public function get_escuelas_desinfeccion($supervision_id) {
		return $this->db->select('escuela.id')
				->from('eleccion_desinfeccion ed')
				->join('escuela', 'ed.escuela_id = escuela.id')
