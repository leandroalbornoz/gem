...
class Datatables {
	...
	/**
	 * Establece columnas para utilizar búsqueda exacta
	 *
	 * @param string $columns
	 * @return mixed
	 */
	public function exact_search($columns) {
		foreach ($this->explode(',', $columns) as $val) {
			$column = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
			$column = preg_replace('/.*\.(.*)/i', '$1', $column);
			$this->exact_search[$column] = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$1', $val));
		}
		return $this;
	}
	 ...
	 * @return mixed
	 */
	public function join($table, $fk, $type = NULL, $escape = TRUE) {
		$this->joins[] = array($table, $fk, $type, $escape);
	...
	public function where($key_condition, $val = NULL, $backtick_protect = TRUE) {
		$this->where[] = array(isset($this->select[$key_condition]) ? $this->select[$key_condition] : $key_condition, $val, $backtick_protect);
		$this->ci->db->where(isset($this->select[$key_condition]) ? $this->select[$key_condition] : $key_condition, $val, $backtick_protect);
		return $this;
	}
	...
	/**
	 * Set fast mode (removes total count)
	 *
	 * @param boolean $fast_mode
	 * @return mixed
	 */
	public function fast_mode($fast_mode = TRUE) {
	...
	private function get_ordering() {
		...
		$custom_order = $this->ci->input->post('sort');

		if ($this->ci->input->post('order'))
			foreach ($this->ci->input->post('order') as $key)
				if (isset($custom_order[$Data[$key['column']]['data']]))
					$this->ci->db->order_by($custom_order[$Data[$key['column']]['data']], $key['dir']);
				else
	...
	private function get_filtering() {
		...
		for ($i = 0; $i < count($mColArray); $i++) {
			if (isset($mColArray[$i]['searchable']) && isset($mColArray[$i]['search']['value'])) {
				if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns) && $mColArray[$i]['search']['value'] != '') {
					if (array_key_exists($mColArray[$i]['data'], $this->exact_search)) {
	...
	private function has_filtering() {
		$mColArray = $this->ci->input->post('columns');
	...
	private function produce_output($output, $charset) {
		...
		if ($output == 'json') {
			if ($this->fast_mode) {
				$iStart = $this->ci->input->post('start');
				$iLength = $this->ci->input->post('length');
				$iTotal = count($aaData) < $iLength ? $iStart + count($aaData) : 1000000;
				$iFilteredTotal = count($aaData) < $iLength ? $iStart + count($aaData) : 1000000;
			} else {
	...
	private function get_total_results($filtering = FALSE) {
		...
		foreach ($this->joins as $val)
			$this->ci->db->join($val[0], $val[1], $val[2], $val[3]);
		...
		if (strlen($this->distinct) > 0) {
			...
			if (empty($this->group_by) && empty($this->having)) {
				$this->ci->db->select('count(1) as filas');
				$query = $this->ci->db->get($this->table)->row();
				return ($query) ? $query->filas : 0;
			}
