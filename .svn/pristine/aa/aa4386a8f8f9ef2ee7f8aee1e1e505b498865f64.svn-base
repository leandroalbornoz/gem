...
class Alumno_model extends MY_Model {
	...
	public function __construct() {
		...
		$this->fields = array(
			...
			'estado_civil' => array('label' => 'Estado civil', 'input_type' => 'combo', 'id_name' => 'estado_civil_id'),
		...
		$this->default_join = array(
			array('persona', 'persona.id = alumno.persona_id', 'left', array('cuil', 'apellido', 'nombre', 'fecha_nacimiento', 'telefono_fijo', 'telefono_movil', 'documento', 'departamento', 'manzana', 'calle', 'piso', 'barrio', 'calle_numero', 'casa', 'codigo_postal', 'email', 'lugar_traslado_emergencia')),
	...
	public function get_alumnos_division($division_id) {
		return $this->db->select("ad.id, al.persona_id, al.observaciones, ad.condicion, ce.descripcion causa_entrada, ad.fecha_desde, ad.fecha_hasta, p.documento, p.documento_tipo_id, CONCAT_WS(', ', p.apellido, p.nombre) as persona, p.fecha_nacimiento, s.descripcion sexo, dt.descripcion_corta as documento_tipo, ad.ciclo_lectivo, ad.condicion")
				->from('alumno al')
	...
	public function get_alumno_escuela($escuela_id = '', $documento = '', $apellido = '', $nombre = '') {
		$this->db->select('p.id as id, al.id as alumno_id, ad.legajo, ad.id as division_id, l.descripcion as localidad, dep.descripcion as departamento,'
				. 'es.id as escuela_id, p.fecha_nacimiento, ad.ciclo_lectivo, c.descripcion as curso, d.division, t.descripcion as turno, '
				. "CONCAT(COALESCE(CONCAT(dt.descripcion_corta,' '),''), COALESCE(CONCAT(p.documento,' '),'')) as documento, "
				. "CONCAT(COALESCE(CONCAT(p.calle,' '),''), COALESCE(CONCAT(p.calle_numero,' '),''), COALESCE(CONCAT('Dpto:',p.departamento,' '),''), COALESCE(CONCAT('P:',p.piso,' '),''), COALESCE(CONCAT('B°:',p.barrio,' '),''), COALESCE(CONCAT('M:',p.manzana,' '),''), COALESCE(CONCAT('C:',p.casa,' '),'')) as direccion, "
			...
			->where('ad.fecha_hasta IS NULL')
			->where('al.id IS NOT NULL')
			->where('ad.ciclo_lectivo', '2018')
			->group_by('ad.id');
		if (!empty($escuela_id)) {
			$this->db->where('es.id', $escuela_id);
		}
		if (!empty($documento)) {
			$this->db->where('p.documento', $documento);
		}
		...
		if (!empty($documento) || !empty($apellido) || !empty($nombre)) {
			return $this->db->get()->result();
		} else {
			return FALSE;
		}
	...
	public function get_alumno_escuela_division($escuela_id = '', $documento = '', $apellido = '', $nombre = '', $division_id = '') {
		$this->db->select('p.id as id, al.id as alumno_id, ad.legajo, ad.id as division_id, l.descripcion as localidad, dep.descripcion as departamento,'
				. 'es.id as escuela_id, p.fecha_nacimiento, ad.ciclo_lectivo, c.descripcion as curso, d.division, t.descripcion as turno, '
				. "CONCAT(COALESCE(CONCAT(dt.descripcion_corta,' '),''), COALESCE(CONCAT(p.documento,' '),'')) as documento, "
				. "CONCAT(COALESCE(CONCAT(p.calle,' '),''), COALESCE(CONCAT(p.calle_numero,' '),''), COALESCE(CONCAT('Dpto:',p.departamento,' '),''), COALESCE(CONCAT('P:',p.piso,' '),''), COALESCE(CONCAT('B°:',p.barrio,' '),''), COALESCE(CONCAT('M:',p.manzana,' '),''), COALESCE(CONCAT('C:',p.casa,' '),'')) as direccion, "
