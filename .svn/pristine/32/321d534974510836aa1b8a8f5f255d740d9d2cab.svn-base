...
class Alumno_apoyo_especial_model extends MY_Model {
	...
	public function get_vista_linea($data) {
		...
		$data_db['totales_apoyo'] = $this->db->select('count(distinct alumno_id) as alumnos_apoyo, count(distinct escuela_id) as escuelas_apoyo')->from('alumno_apoyo_especial')->get()->row();
		return $data_db;
	}
