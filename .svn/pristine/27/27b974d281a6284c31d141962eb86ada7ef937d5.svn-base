<div class="content-wrapper">
	<section class="content-header">
		<h1>
			Cargos cerrados de Esc. <?= $escuela->nombre_corto; ?>
		</h1>
		<ol class="breadcrumb">
			...
			<div class="col-xs-12">
				<div class="box box-primary">
					<div class="box-body">
						...
						<a class="btn btn-app btn-app-zetta active btn-app-zetta-active" href="cargo/listar_cargosc/<?php echo $escuela->id; ?>">
...
<script>
	var cargoc_table;
	function complete_cargoc_table() {
		$('#cargoc_table tfoot th').each(function(i) {
			var title = $('#cargoc_table thead th').eq(i).text();
			if (title !== '' && title !== 'Servicios') {
				$(this).html('<input class="form-control input-xs" style="width: 100%;" type="text" placeholder="' + title + '" value="' + cargoc_table.column(i).search() + '"/>');
			}
		});
		$('#cargoc_table tfoot th').eq(10).html('<button class="btn btn-xs btn-default" onclick="limpiar_filtro(\'cargoc_table\');" title="Limpiar filtros"><i class="fa fa-eraser"></i> Filtros</button>');
		cargoc_table.columns().every(function() {
			var column = this;
			$('input', cargoc_table.table().footer().children[0].children[this[0][0]]).on('change keypress', function(e) {
		...
		var r = $('#cargoc_table tfoot tr');
		r.find('th').each(function() {
			$(this).css('padding', 8);
		});
		$('#cargoc_table thead').append(r);
		$('#search_0').css('text-align', 'center');
	}
	function cargoc_table_detalles(api, rowIdx, columns) {
		...
		html += '<div id="cargoc_table_detalle_' + rowIdx + '"><i class="fa fa-spin fa-refresh"></i>Cargando servicios...</div>';
		$.ajax({
			type: 'GET',
			...
			success: function(result) {
				$('#cargoc_table_detalle_' + rowIdx).html(format(cargo_id, result));
			}
		});
		return html;
	}
