...
//						), FALSE);
	...
	public function escritorio($id) {
		...
		if ($supervision->nivel_id === '2' && $supervision->dependencia_id === '1') {
			...
			if (!empty($escuelas_preinscripcion)) {
				...
				$data['modulos']['escuelas_preinscripcion'] = $this->load->view('preinscripciones/escritorio_supervision_modulo_preinscripciones', $this->preinscripcion_model->get_vista_supervision($supervision->id, $data), TRUE);
			}
		}
