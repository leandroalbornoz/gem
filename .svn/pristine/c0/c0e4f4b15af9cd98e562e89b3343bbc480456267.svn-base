...
class Personal extends MY_Controller {
	...
	public function agregar($area_id = NULL, $mes = NULL) {
		...
		$this->array_situacion_revista_control = $array_situacion_revista = $this->get_array('situacion_revista', 'descripcion', 'id', array('where' => array('id IN (1,2,3)')), array('' => '-- Seleccionar S.R --'));
		...
		$this->servicio_model->fields['carga_horaria_cargo'] = array('label' => 'Carga Horaria semanal', 'type' => 'number');
		$this->load->model('funcion_model');
		$this->array_funcion_control = $array_funcion = $this->get_array('funcion', 'descripcion', 'id', array(
			'planilla_modalidad_id' => 1,
			), array('' => '-- Seleccionar función --'));
		unset($this->array_funcion_control['']);
		...
		if (isset($_POST) && !empty($_POST)) {
			if ($this->input->post('tipo_regimen') === 'Hora') {
				$this->servicio_model->fields['carga_horaria_cargo']['required'] = TRUE;
			}
			...
			if ($this->form_validation->run() === TRUE) {
				...
				if ($trans_ok && $persona_id) {
					$trans_ok &= $this->cargo_model->create(array(
						...
						'area_id' => $area->id,
						'carga_horaria' => $this->input->post('carga_horaria_cargo'),
	...
	public function editar($id = NULL) {
		...
		if (empty($servicio->area_id)) {
			redirect("servicio/ver/$servicio->id");
		}
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			redirect("areas/personal/editar/$servicio->id", 'refresh');
		}

		$fields_servicio2 = $this->build_fields($model->fields, $servicio);
		...
		$data['fields_servicio2'] = $fields_servicio2;
		$data['error'] = $this->session->flashdata('error');
		...
		$data['title'] = TITLE . ' - Editar personal';
	...
	public function eliminar($id = NULL) {
		...
		$data['title'] = TITLE . ' - Eliminar personal';
		$this->load_template('areas/personal/personal_abm', $data);
	}
	...
	public function ver($id = NULL) {
		...
		$this->load->model('servicio_novedad_model');
		$servicio_novedades = $this->servicio_novedad_model->get_servicio_novedades_area($servicio->id, $servicio->persona_id, $area->id);
	...
	public function ver_funcion($id = NULL) {
		...
		$data['txt_btn'] = NULL;
		$data['class'] = array('agregar' => '', 'ver' => 'active btn-app-zetta-active', 'editar' => '', 'eliminar' => '');
		$data['title'] = TITLE . ' - Ver función personal';
		$this->load_template('areas/personal/personal_abm', $data);
	}
	...
	public function horarios($id = NULL) {
		...
		$horarios_car_db = $this->horario_model->get(array('cargo_id' => $servicio->cargo_id));
		...
		foreach ($dias as $dia) {
			if (!empty($horarios_div)) :
				if (isset($horarios_div[$dia->id])):
					foreach ($horarios_div[$dia->id] as $cargo_horario) {
						$cd = ($cargo_horario->curso . " - " . $cargo_horario->division);
					}
					foreach ($horarios_div[$dia->id] as $cargo_horario) {
						if ($cd == ($cargo_horario->curso . " - " . $cargo_horario->division)) {
							$bool_cd = TRUE;
						}
					}
				endif;
		...
		$data['error'] = $this->session->flashdata('error');
		$data['servicio_funciones'] = $servicio_funciones;
		$data['servicio_funcion'] = $servicio_funciones;
		$data['servicio'] = $servicio;
		$data['cargo'] = $cargo;
		$data['area'] = $area;
		$data['txt_btn'] = NULL;
		$data['class'] = array('agregar' => '', 'ver' => '', 'editar' => '', 'eliminar' => '');
		$data['title'] = TITLE . ' - Ver horarios personal';
		$this->load_template('areas/personal/cargo_horarios', $data);
	}

	public function mover($servicio_id, $area_id) {
		if (!in_array($this->rol->codigo, $this->roles_admin) || $servicio_id == NULL || $area_id == NULL || !ctype_digit($servicio_id) || !ctype_digit($area_id)) {
		...
		$this->servicio_model->fields['area']['label'] = 'Área origen';
		$this->servicio_model->fields['area']['value'] = $servicio->area_codigo . ' ' . $servicio->area;
		$servicio->nombre = "$servicio->apellido, $servicio->nombre";
		...
		$model = new stdClass();
		$model->fields = array(
			'area_destino' => array('label' => 'Área destino', 'input_type' => 'combo'),
		);
		$this->load->model('areas/area_model');
		$this->array_area_destino_control = $array_area_destino = $this->get_array('area', 'area', 'id', array('select' => array('id', "CONCAT(codigo, ' - ', descripcion) as area")), array('' => '-- Seleccionar Área destino --'));
		$model->fields['area_destino']['array'] = $array_area_destino;

		$this->set_model_validation_rules($model);
		if (isset($_POST) && !empty($_POST)) {
			if ($this->form_validation->run() === TRUE) {
				if ($servicio_id !== $this->input->post('id')) {
					show_error('Esta solicitud no pasó el control de seguridad.');
				}
				$trans_ok = TRUE;
				$this->load->model('cargo_model');
				$trans_ok &= $this->cargo_model->update(array(
					'id' => $servicio->cargo_id,
					'area_id' => $this->input->post('area_destino')
				));
				if ($trans_ok) {
					$this->session->set_flashdata('message', $this->cargo_model->get_msg());
					redirect("areas/personal/ver/$servicio->id", 'refresh');
				}
			} else {
				$this->session->set_flashdata('error', validation_errors());
				redirect("areas/personal/mover/$servicio->id/$area->id", 'refresh');
			}
		}
		$fields_servicio = $this->build_fields($this->servicio_model->fields, $servicio, TRUE);
		$fields_cargo = $this->build_fields($this->cargo_model->fields, $cargo, TRUE);
		...
		$data['servicio'] = $servicio;
		$data['error'] = $this->session->flashdata('error');
		$data['fields_servicio'] = $this->build_fields($this->servicio_model->fields, $servicio, TRUE);
		$data['fields_destino'] = $this->build_fields($model->fields);
		$data['txt_btn'] = 'Mover';
		$data['class'] = array('agregar' => '', 'ver' => '', 'editar' => '', 'eliminar' => '', 'mover' => 'active btn-app-zetta-active');
		$data['title'] = "Mover personal";
		$this->load_template('personal/personal_mover', $data);
	}
}
