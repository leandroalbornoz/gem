...
class Persona_model extends MY_Model {
	...
	public function get_by_documento($documento) {
		return $this->db->select('id, nombre, apellido')->from('persona')->where('documento', $documento)->get()->row();
	}
	...
	function get_persona_parcial($opcion, $parametro) {
		$this->db->select('persona.id, persona.cuil, persona.apellido, persona.nombre, persona.fecha_nacimiento, persona.sexo_id, persona.calle, persona.calle_numero, persona.piso, persona.departamento, persona.telefono_fijo, persona.telefono_movil, persona.codigo_postal, persona.email, persona.documento, CONCAT(departamento.descripcion, " - ", localidad.descripcion) as localidad, documento_tipo.descripcion_corta as documento_tipo, sexo.descripcion as sexo, persona.localidad_id, sexo.descripcion')
			->from('persona')
		...
		if ($opcion === 'cuil'):
			$this->db->where('persona.cuil', $parametro);
		elseif ($opcion === 'id'):
			$this->db->where('persona.id', $parametro);
		endif;
		$query = $this->db->get();
		return $query->row();
	}
