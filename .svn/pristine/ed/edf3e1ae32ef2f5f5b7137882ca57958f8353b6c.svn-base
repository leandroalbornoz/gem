										...
										<td style="vertical-align: top;">
											<?php if (isset($horarios_car[$dia->id])): ?>
												<?php foreach ($horarios_car[$dia->id] as $position => $cargo_horario) : ?>
													<?php echo substr($cargo_horario->hora_desde, 0, 5) . ' - ' . substr($cargo_horario->hora_hasta, 0, 5).($cargo_horario->obligaciones === '1.0' ? '' : '<br/>(' . $cargo_horario->obligaciones . ' oblig.)') . '<br/>';  ?>
