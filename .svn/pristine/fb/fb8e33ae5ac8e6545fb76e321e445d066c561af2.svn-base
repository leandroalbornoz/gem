...
//		else { $this->ktBlock[$this->page]['bottom_margin'] = $this->y+$this->divheight; }
	...
	if($txt!='') {
		...
		// OUTLINE
		if(isset($this->textparam['outline-s']) && $this->textparam['outline-s'] && !$this->S) {	// mPDF 5.6.07
...
function WriteFlowingBlock( $s)
{
   ...
   for ( $i = 0; $i < $tmp; $i++ )  {
	...
	if (( $contentWidth + $cw > $maxWidth - $WidthCorrection - (($this->cMarginL+$this->cMarginR)*_MPDFK) - ($paddingL+$paddingR +(($fpaddingL + $fpaddingR) * _MPDFK) ) +  0.001))  {// 0.001 is to correct for deviations converting mm=>pts
		...
		$hardsuccess = false;
		if (isset($this->textparam['hyphens']) && $this->textparam['hyphens'] != 2 && preg_match("/\-/",$currWord)) {	
...
function _putcatalog() {
	...
	// mPDF 5.6.01
	if ((isset($this->hasOC) && $this->hasOC) || count($this->layers)) {
...
function OpenTag($tag,$attr)
{
  ...
  switch($tag){
	...
	if (!isset($this->restoreBlockPageBreaks))
...
function CloseTag($tag)
{
	...
	if (count($this->textbuffer) == 0 && $this->lastblocklevelchange != 0) {
		...
		$this->newFlowingBlock( $this->blk[$this->blklvl]['width'],$this->lineheight,'',false,false,$blockstate,true, 'ltr');
		$this->finishFlowingBlock(true);	// true = END of flowing block
   ...
   for($cctr=0;$cctr<count($textbuffer);$cctr++) {	// mPDF 5.6.22
	...
	foreach ($words as $k=>$word) {
		...
		// Word does not fit on line...
				...
				// mPDF 5.6.21  hard hyphens
				if (isset($this->textparam['hyphens']) && $this->textparam['hyphens'] != 2 && preg_match('/\-/',$word)) {
...
function _putresources() {
	if ((isset($this->hasOC) && $this->hasOC) || count($this->layers)) 	// mPDF 5.6.01
		$this->_putocg();
	$this->_putextgstates();
	$this->_putspotcolors();
	$this->_putfonts();
	...
	$this->_out('/Font <<');
	foreach($this->fonts as $font) {
		if (isset($font['used']) && !$font['used'] && $font['type']=='TTF') { continue; }
		if ($font['type']=='TTF' && ($font['sip'] || $font['smp'])) {
			foreach($font['n'] AS $k => $fid) {
				$this->_out('/F'.$font['subsetfontids'][$k].' '.$font['n'][$k].' 0 R');
			}
