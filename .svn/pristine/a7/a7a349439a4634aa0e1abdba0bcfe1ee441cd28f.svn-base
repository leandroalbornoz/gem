...
class Preinscripcion_model extends MY_Model {
	...
	public function get_vista($escuela_id, $ciclo_lectivo, $data) {
		...
		$preinscipcion = $this->db->select("p.id, p.escuela_id, p.vacantes, p.ciclo_lectivo, "
					. "COALESCE(SUM(CASE pa.estado WHEN 'Inscripto' THEN 1 ELSE 0 END),0) inscriptos, "
					. "COALESCE(SUM(CASE pa.estado WHEN 'Postulante' THEN 1 ELSE 0 END), 0) postulantes, "
					. "COALESCE(SUM(CASE pa.estado WHEN 'Derivado' THEN 1 ELSE 0 END), 0) derivados, "
					. "COALESCE(SUM(CASE WHEN pa.preinscripcion_tipo_id <= 3 AND pa.estado = 'Inscripto' THEN 1 ELSE 0 END),0) instancia_1, "
				...
				->group_by('p.id')
				->get()->row();
		$return['preinscripcion'] = $preinscipcion;
		return $return;
	}
	...
	public function get_vista_supervision($supervision_id, $data) {
		...
		$return['administrar'] = $data['administrar'];
		$escuelas_p = $this->db->select('e.id, e.numero, e.anexo, e.nombre, COUNT(DISTINCT d.id) divisiones, COUNT(DISTINCT ad.id) alumnos, p.id as preinscripcion_id, p.vacantes, pa.inscriptos, pa.postulantes, pa.instancia_1, pa.instancia_2, pa.instancia_3, pa.instancia_2_p, pa.instancia_2_i, pa.instancia_3_d, pa.instancia_4')
				->from('escuela e')
				->join('supervision s', 'e.supervision_id = s.id')
