...
class Persona extends MY_Controller {
	...
	public function listar_data() {
		...
		$this->datatables
			->fast_mode()
			->select("persona.id, persona.cuil, documento_tipo.descripcion_corta documento_tipo, persona.documento, CONCAT(persona.apellido, ', ', persona.nombre) as persona, CONCAT(COALESCE(CONCAT(persona.calle,' '),''), COALESCE(CONCAT(persona.calle_numero,' '),''), COALESCE(CONCAT('Dpto:',persona.departamento,' '),''), COALESCE(CONCAT('P:',persona.piso,' '),''), COALESCE(CONCAT('B°:',persona.barrio,' '),''), COALESCE(CONCAT('M:',persona.manzana,' '),''), COALESCE(CONCAT('C:',persona.casa,' '),'')) as direccion, localidad.descripcion as localidad, CONCAT(persona.telefono_fijo,' ', persona.telefono_movil) as telefonos")
