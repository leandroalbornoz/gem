...
class Escuela_model extends MY_Model {
	...
	public function get_by_nivel($nivel_id, $dependencia = 0, $supervision = 0, $regional = 0) {
		$options['join'] = array(
			array("(SELECT e.id as escuela_id,
	...
	where e.nivel_id ='" . $nivel_id . "' and COALESCE(ad.fecha_hasta,'2017-12-01')>='2017-12-01'
	GROUP BY e.id,e.anexo
) ad ", " ad.escuela_id=escuela.id", 'left', array("ad.porcentaje_actualizacion_cl", "
	ad.matricula_2017", "ad.matricula_2018", "ad.diferencia_año_2018_2017")),
			array("(SELECT c.escuela_id, COUNT(DISTINCT c.id) cargos FROM cargo c JOIN regimen r ON c.regimen_id = r.id WHERE c.fecha_hasta IS NULL AND r.planilla_modalidad_id = 1 GROUP BY c.escuela_id) c", 'c.escuela_id = escuela.id', 'left', array('c.cargos')),
