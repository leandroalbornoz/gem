...
class Validacion extends MY_Controller {
	...
	public function estadisticas() {
		...
		$data['estadisticas'] = $this->beca_model->get_estadisticas(1);
		$data['error'] = $this->session->flashdata('error');
		$data['message'] = $this->session->flashdata('message');
		$data['title'] = 'Becas - Estadísticas';
		$data['vw_etapas'] = $this->load->view('becas/validacion/validacion_etapas', array('etapas' => $this->beca_etapa_model->get()), TRUE);
		$data['css'][] = 'plugins/c3/c3.min.css';
		$data['js'][] = 'plugins/d3/d3.min.js';
		$data['js'][] = 'plugins/c3/c3.min.js';
		$this->load_template('becas/validacion/validacion_estadisticas', $data);
	}
}
