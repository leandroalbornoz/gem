...
class Abono_alumno_model extends MY_Model {
	...
	public function __construct() {
		...
		$this->id_name = 'id';
		$this->columnas = array('id', 'alumno_id', 'escuela_id', 'abono_alumno_estado_id', 'abono_tipo_id', 'abono_motivo_alta_id', 'fecha_alta', 'numero_abono', 'monto', 'ames');
		$this->fields = array(
		...
		$this->default_join = array(
			...
			array('abono_alumno_estado', 'abono_alumno_estado.id = abono_alumno.abono_alumno_estado_id'),
		);
	}
	...
	public function get_suma_monto_mes($escuela_id, $ames) {
		return $this->db->select('SUM(monto) as monto_escuela_ames')
				->from('abono_alumno aa')
				->where('aa.ames', $ames)
				->where('aa.escuela_id', $escuela_id)
	...
	public function get_suma_cupos_mes($escuela_id, $ames) {
		return $this->db->select('count(aa.id) as cupos_escuela_ames')
				->from('abono_alumno aa')
				->where('aa.ames', $ames)
				->where('aa.escuela_id', $escuela_id)
				->get()->row();
	...
	public function get_cantidad_alumnos_espera($escuela_id, $ames) {
		return $this->db->select('count(*) as cantidad')
				...
				->where('aa.ames', $ames)
				->where('aa.escuela_id', $escuela_id)
				->where('aa.abono_alumno_estado_id', 2)
				->get()->row();
	}

	public function get_abonos_escuela($escuela_id) {
		$datos = $this->db->select('*')
				->from('abono_escuela_monto aem')
				->where('aem.escuela_id', $escuela_id)
				->order_by('aem.ames DESC')
				->get()->row();
		return $datos;
	}
	public function get_abonos_escuela_ames($escuela_id,$ames) {
		$datos = $this->db->select('*')
				->from('abono_escuela_monto aem')
				->where('aem.escuela_id', $escuela_id)
				->where('aem.ames', $ames)
