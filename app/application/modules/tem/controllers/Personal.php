...
//		$this->nav_route = 'menu/servicio';
	...
	public function listar($escuela_id = NULL, $mes = NULL) {
		...
		$proyecto_db = $this->tem_proyecto_escuela_model->get(array(
			...
			'select' => 'horas_catedra, tem_mes_semana.mes, tem_proyecto.fecha_desde, tem_proyecto.fecha_hasta',
			'join' => array(
				array('tem_mes_semana', 'tem_mes_semana.tem_proyecto_id = tem_proyecto_escuela.tem_proyecto_id'),
				array('tem_proyecto', 'tem_proyecto.id = tem_proyecto_escuela.tem_proyecto_id'),
			),
		...
		$altas = array();
		$bajas = array();
		$inicio = new DateTime($mes . '14');
		$fin = new DateTime($mes . '01 +1 month');
		if ($proyecto->fecha_hasta < $fin->format('Y-m-d')) {
			$fin = new DateTime($proyecto->fecha_hasta . ' +1 day');
		}
		...
		if (!empty($cargos)) {
			foreach ($cargos as $cargo) {
				...
				if ($cargo->fecha_alta >= $inicio->format('Y-m-d')) {
		...
		if (empty($cargos) || (empty($altas) && empty($bajas))) {
	...
	public function agregar($escuela_id = NULL, $mes = NULL) {
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			$this->servicio_model->fields['fecha_baja']['value'] = '28/02/2018';
			$this->servicio_model->fields['fecha_baja']['readonly'] = TRUE;

			$this->load->model('tem_proyecto_model');
			$proyecto = $this->tem_proyecto_model->get_proyecto_mes($mes);
			if ((new DateTime($proyecto->fecha_hasta))->format('Ym') === $mes &&
				$this->input->post('fecha_hasta') >= $proyecto->fecha_hasta) {
				$data['fecha_hasta'] = (new DateTime($proyecto->fecha_hasta . ' -1 day'))->format('d/m/Y');
			} else {
	...
	public function agregar_personal($escuela_id = NULL, $mes = NULL) {
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			if ($fecha_alta->format('Ym') != $mes_actual && $this->usuario !== '37') {
