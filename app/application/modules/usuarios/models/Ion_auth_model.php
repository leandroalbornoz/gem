...
class Ion_auth_model extends CI_Model {
	 ...
	 * @var string
	 * */
	public $codigo_contrasena_olvidada;
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		$query = $this->db->select('contrasena, semilla_contrasena')
			->where('id', $id)
			->limit(1)
			->order_by('id', 'desc')
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			if ($this->bcrypt->verify($password, $hash_password_db->contrasena))
			{
				return TRUE;
			}
		...
		if ($this->store_salt)
		{
			$db_password = sha1($password . $hash_password_db->semilla_contrasena);
		}
		else
		{
			$salt = substr($hash_password_db->contrasena, 0, $this->salt_length);

			$db_password = $salt . substr(sha1($salt . $password), 0, -$this->salt_length);
		}

		if ($db_password == $hash_password_db->contrasena)
		{
			return TRUE;
		}
		else
	...
	public function deactivate($id = NULL)
	{
		...
		$codigo_activacion = sha1(md5(microtime()));
	...
	public function clear_forgotten_password_code($code)
	{
		...
		$this->db->where('codigo_contrasena_olvidada', $code);

		if ($this->db->count_all_results($this->tables['users']) > 0)
		{
			$data = array(
				'codigo_contrasena_olvidada' => NULL,
				'hora_contrasena_olvidada' => NULL
			);

			$this->db->update($this->tables['users'], $data, array('codigo_contrasena_olvidada' => $code));
	...
	public function forgotten_password($identity)
	{
		...
		$codigo_activacion_part = "";
		if (function_exists("openssl_random_pseudo_bytes"))
		{
			$codigo_activacion_part = openssl_random_pseudo_bytes(128);
		}

		for ($i = 0; $i < 1024; $i++)
		{
			$codigo_activacion_part = sha1($codigo_activacion_part . mt_rand() . microtime());
		}

		$key = $this->hash_code($codigo_activacion_part . $identity);
		...
		$this->codigo_contrasena_olvidada = $key;

		$this->trigger_events('extra_where');

		$update = array(
			'codigo_contrasena_olvidada' => $key,
			'hora_contrasena_olvidada' => time()
		);
	...
	public function forgotten_password_complete($code, $salt = FALSE)
	{
		...
		$profile = $this->where('codigo_contrasena_olvidada', $code)->users()->row(); //pass the code to profile
		...
		if ($profile)
		{
			...
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0)
			{
				...
				if (time() - $profile->hora_contrasena_olvidada > $expiration)
				{
					//it has expired
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select('usuario.id, usuario, usuario.email, contrasena, activo, ultimo_acceso, p.cuil, p.apellido, p.nombre, foto')
			->join('persona p', 'p.id=usuario.persona_id')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
	...
	public function get_attempts_num($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			{
				$this->db->where('direccion_ip', $ip_address);
				$this->db->where('usuario', $identity);
			}
			else if (strlen($identity) > 0)
				$this->db->or_where('usuario', $identity);
	...
	public function increase_login_attempts($identity)
	{
		...
		{
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			return $this->db->insert($this->tables['login_attempts'], array('direccion_ip' => $ip_address, 'usuario' => $identity, 'hora' => time()));
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $expire_period = 86400)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			$this->db->where(array('direccion_ip' => $ip_address, 'usuario' => $identity));
			// Purge obsolete login attempts
			$this->db->or_where('hora <', time() - $expire_period, FALSE);
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) || array_key_exists('contrasena', $data) || array_key_exists('email', $data))
		{
			if (array_key_exists('contrasena', $data))
			{
				if (!empty($data['contrasena']))
				{
					$data['contrasena'] = $this->hash_password($data['contrasena'], $user->semilla_contrasena);
				}
				else
				{
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('ultimo_acceso' => time()), array('id' => $id));
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select('usuario.id, usuario, usuario.email, ultimo_acceso, p.cuil, p.apellido, p.nombre, foto')
			->join('persona p', 'p.id=usuario.persona_id')
