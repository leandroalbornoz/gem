...
class Escuela extends MY_Controller {
	...
	public function ver($comedor_presupuesto_id = NULL) {
		...
		$fecha_c = $this->comedor_plazo_model->get(array('mes' =>$comedor_presupuesto->mes));
	...
	public function modal_importar_alumnos($comedor_presupuesto_id = NULL) {
		...
		$alumnos_importar = $this->comedor_alumno_model->get(array(
			'join' => array(array('alumno_division ad', "ad.id = comedor_alumno.alumno_division_id AND (ad.fecha_hasta IS NULL OR DATE_FORMAT(ad.fecha_hasta,'%Y%m') >= {$comedor_presupuesto->mes})")),
			'comedor_presupuesto_id' => $comedor_presupuesto_anterior[0]->id
		));
		$alumnos_actuales = $this->comedor_alumno_model->get(array('comedor_presupuesto_id' => $comedor_presupuesto_id));
		if (empty($alumnos_importar)) {
			$this->modal_error('No hay alumnos para importar', 'Sin alumnos en mes anterior');
			return;
		}
		if (!empty($alumnos_actuales)) {
			$alumnos_imp = array();
			foreach ($alumnos_importar as $alumno) {
				$alumnos_imp[] = $alumno->alumno_division_id;
			}
			$alumnos_act = array();
			foreach ($alumnos_actuales as $alumno) {
				$alumnos_act[] = $alumno->alumno_division_id;
			}
			$alumnos_agregar = array_diff($alumnos_imp, $alumnos_act);
			if (empty($alumnos_agregar)) {
				$this->modal_error('Todos los alumnos ya fueron importados', 'Acción no autorizada');
				return;
			}
		}
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			foreach ($alumnos_importar as $alumno) {
				if (!empty($alumnos_actuales)) {
					if (in_array($alumno->alumno_division_id, $alumnos_agregar)) {
