...
where id= ?", array($comedor_presupuesto_id)
	...
	public function get_alumnos_division($division_id, $id = NULL, $ames) {
		if (empty($id) || !ctype_digit($id)) {
			$alumnos_db = $this->db->select('ad.id, al.persona_id, al.observaciones, ad.fecha_desde, ad.fecha_hasta, '
					...
					->where("(ad.fecha_hasta IS NULL OR DATE_FORMAT(ad.fecha_hasta,'%Y%m') >= $ames)")
					->where("ad.ciclo_lectivo", substr($ames, 0, 4))
