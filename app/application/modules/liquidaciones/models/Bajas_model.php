...
class Bajas_model extends CI_Model {
	...
	public function get_by_departamento($mes) {
		$bajas = $this->db->query("SELECT sr.planilla_tipo_id, COALESCE(d.id,0) id, COALESCE(d.descripcion, 'N/D') departamento, COUNT(DISTINCT e.id) escuelas_bajas, COUNT(DISTINCT sn_baja.id) bajas_cargadas, COUNT(DISTINCT CASE WHEN sn_baja.estado='Cargado' THEN NULL ELSE sn_baja.id END) bajas_auditadas
FROM servicio_novedad sn_baja
JOIN novedad_tipo nt ON sn_baja.novedad_tipo_id = nt.id
JOIN servicio s ON sn_baja.servicio_id = s.id
...
JOIN regimen r ON c.regimen_id = r.id AND r.planilla_modalidad_id = 1
JOIN escuela e ON c.escuela_id = e.id AND e.dependencia_id = 1
WHERE e.id=? AND sr.planilla_tipo_id=? AND sn.ames=? AND nt.novedad = 'B' AND sn.planilla_baja_id IS NULL
GROUP BY s.id
ORDER BY p.documento", array($escuela, $planilla_tipo_id, $mes))->result();
	}

	public function get_bajas($planilla_tipo_id, $mes) {
