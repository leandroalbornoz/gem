<?php echo form_open(uri_string(), array('data-toggle' => 'validator', 'id' => 'form_buscar_encargado_desinfeccion')); ?>
...
<script>
	...
	$(document).ready(function() {
		...
		agregar_eventos($('#form_buscar_encargado_desinfeccion'));
		...
		$('#btn-search').click(function() {
			...
			if (cuil !== '') {
				...
				if (cuil !== '') {
					$.ajax({
						type: 'GET',
						url: 'conectividad/instalacion/ajax_listar_encargados?',
						data: {cuil: cuil, escuela_id: <?php echo $conectividad_escuela->escuela_id;?>},
						dataType: 'json',
								...
								if (result.personas_listar.length > 0) {
									for (var idx in result.personas_listar) {
										...
										table_personas_busqueda.row.add([
											...
											'<a class="btn btn-xs btn-success pull-right" href="conectividad/instalacion/agregar_encargado/<?php echo "$conectividad_escuela->id"; ?>/' + personas_listar.servicio_id + '"><i class="fa fa-plus"></i> Agregar encargado</a>']);
									}
								}
