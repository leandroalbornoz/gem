...
class Carrera extends MY_Controller {
	...
	function __construct() {
		...
		if (!in_array($this->rol->codigo, array(ROL_ADMIN, ROL_USI, ROL_LINEA, ROL_GRUPO_ESCUELA))) {
			$this->edicion = FALSE;
		}
	...
	public function listar_data($nivel_id = '') {
		...
		switch ($this->rol->codigo) {
			...
			case ROL_GRUPO_ESCUELA_CONSULTA:
				$this->datatables
					->join('escuela_carrera', 'escuela_carrera.carrera_id=carrera.id')
					->join('escuela_grupo_escuela', 'escuela_grupo_escuela.escuela_id = escuela_carrera.escuela_id')
					->where('escuela_grupo_escuela.escuela_grupo_id', $this->rol->entidad_id)
					->group_by('carrera.id');
				break;
			case ROL_LINEA:
