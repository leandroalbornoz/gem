...
class Curso extends MY_Controller {
	...
	public function listar() {
		...
		$tableData = array(
			'columns' => array(
				...
				array('label' => 'Descripción', 'data' => 'descripcion', 'width' => 61),
				array('label' => 'Grado Múltiple', 'data' => 'grado_multiple', 'width' => 7),
	...
	public function listar_data() {
		...
		$this->datatables
			->select('curso.id, curso.nivel_id, curso.descripcion, curso.grado_multiple, nivel.descripcion as nivel')
			->unset_column('id')
			->from('curso')
	...
	public function modal_agregar() {
		...
		$this->array_grado_multiple_control = $this->curso_model->fields['grado_multiple']['array'];
		...
		if (isset($_POST) && !empty($_POST)) {
			if ($this->form_validation->run() === TRUE) {
				...
				$trans_ok &= $this->curso_model->create(array(
					...
					'grado_multiple' => $this->input->post('grado_multiple')
				));
