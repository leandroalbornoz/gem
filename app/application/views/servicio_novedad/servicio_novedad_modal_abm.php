...
<?php endif; ?>
	...
	function cambia_novedad_tipo() {
		if ($.inArray($('#novedad_tipo').val(), novedades_concomitantes) !== -1) {
			...
			$('#cal_desde,#cal_hasta').datepicker('setEndDate', '<?php echo $fecha_hasta; ?>');
...
<?php if ($operacion === 'Confirmar p'): ?>
			if (input_desde.val() !== '<?php echo $fecha_desde; ?>') {
				$('#alerta').html('No puede modificar fecha desde');
				$('#btn_submit_novedad').attr('disabled', true);
				return false;
			}
<?php endif; ?>
...
<div class="modal-footer">
	...
	echo form_hidden('url_redireccion', $url_redireccion);
	?>
</div>
<?php echo form_close(); ?>