									...
									<?php foreach ($servicios as $servicio): ?>
										<tr class="<?= empty($servicio->fecha_baja) ? '' : 'bg-gray'; ?>">
											...
											<td>
												...
												<?php if ($servicio->dependencia_id === '1' || !empty($servicio->area_id)): ?>
													<?php if (empty($servicio->tbcabh_id) && (empty($servicio->fecha_alta) || (new DateTime($servicio->fecha_alta . ($servicio->situacion_revista === 'Reemplazo' ? '' : ' -1 month')))->format('Ym') < $AMES_LIQUIDACION)): ?>
														<i class="fa fa-times text-red"></i>
														<?php if (empty($servicio->fecha_baja) || (new DateTime(substr($servicio->fecha_baja, 0, 8) . '01' . ($servicio->situacion_revista === 'Reemplazo' ? ' +1 month' : '')))->format('Ymd') >= $AMES_LIQUIDACION . '01'): ?>
															<a class="btn btn-xs btn-primary" href="tbcabh/modal_asignar_liquidacion/<?= "$servicio->id/$AMES_LIQUIDACION"; ?>" data-remote="false" data-toggle="modal" data-target="#remote_modal"><i class="fa fa-search"></i></a>
														...
														<?php endif; ?>
													<?php elseif (!empty($servicio->tbcabh_id)): ?>
											...
											<td>
												...
												<?php if ($this->usuario === '37'): ?>
													<a class="pull-right btn btn-xs btn-danger" href="tbcabh/limpiar_liquidacion_servicio/<?= "$servicio->id"; ?>"><i class="fa fa-ban"></i></a>
												<?php endif; ?>
											</td>
										...
										<tr class="<?= $liquidacion->vigente === $AMES_LIQUIDACION ? '' : 'bg-gray'; ?>">
											<td>
												<?php if (empty($liquidacion->servicio_id)): ?>
													...
													<?php endif; ?>
												<?php elseif ($liquidacion->servicio_id === '-1'): ?>
											...
											<td>
												<?php if ($liquidacion->servicio_id !== '-1'): ?>
													<a href="tbcabh/tbcabh_no_valido/<?= "$liquidacion->id/$liquidacion->vigente/$persona->id"; ?>" class="btn btn-xs btn-danger"><i class="fa fa-ban"></i></a>
												<?php else: ?>
													<a class="btn btn-xs btn-danger" href="tbcabh/desasignar_servicio/<?= "$liquidacion->id/$liquidacion->vigente/$persona->id"; ?>"><i class="fa fa-remove"></i></a>
												<?php endif; ?>
